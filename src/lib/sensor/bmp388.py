...
class DFRobot_BMP388:
    ...
    def __init__(self):
        ...
        self.par_p11 = 0
        self.addr = 119
        self.chip_id = self.bmp3_get_regs(0x00, 1)[0]
        print(hex(self.chip_id))
        if (chip_id != 0x50):
            print("chip id error!")
            sys.exit()
        self.get_calib_data()
    ...
    def readCalibratedAltitude(self, seaLevel):
        return round((1.0 - pow(self.readPressure() / seaLevel, 0.190284)) * 287.15 / 0.0065, 2)
    
    def readSeaLevel(self, altitude):
        return round(self.readPressure() / pow(1.0 - (altitude / 44330.0), 5.255), 2)
    
    def readAltitude(self):
        return round((1.0 - pow(self.readPressure() / 101325, 0.190284)) * 287.15 / 0.0065, 2)
...
class DFRobot_BMP388_SPI(DFRobot_BMP388):
    ...
    def bmp3_get_regs(self, reg, len):
        ...
        rslt = self.spi.take_measurement()
        self.cs.value(1)
        data = bytearray(len)
...
class DFRobot_BMP388_I2C(DFRobot_BMP388):
    ...
    def bmp3_get_regs(self, reg, length):
